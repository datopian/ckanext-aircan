...
class AirflowClient(object):
    ...
    def __init__(self, config, payload=None):
        ...
        self.server_type = (
            config.get("ckan.airflow.cloud", "local") or "local"
        ).upper()
        self.api_version = config.get(
            "ckan.airflow.api_version", self._default_api_version()
        )
        ...
        if self.server_type == "GCP":
            local_config_str = config.get(
                "ckan.airflow.cloud.google_application_credentials"
            )
    ...
    def _get_composer_env_url(self):
        ...
        composer_environment = self.config.get(
            "ckan.airflow.cloud.composer_environment"
        )
    ...
    def _get_composer3_webserver_url(self):
        resp = self._authed_session.get(
            self._get_composer_env_url(), timeout=self.timeout
        )
    ...
    def _login_local(self):
        if not self.username or not self.password:
            raise ValueError(
                "Missing config: ckan.airflow.username / ckan.airflow.password"
            )
    ...
    def get_dag_run(self, dag_run_id):
        endpoint = "/api/{}/dags/{}/dagRuns/{}".format(
            self.api_version, self.dag_id, dag_run_id
        )
    ...
    def get_gcp_logs(self, dag_name):
        ...
        composer_environment = self.config.get(
            "ckan.airflow.cloud.composer_environment", "aircan-airflow"
        )
        client = gcp_logging.Client(
            project_id, credentials=self._authed_session.credentials
        )
        entries_filter = (
